...
def startvideodownload(url=None, extraInfo=""):
    ...
    print("[downloadvideo] Download done! Generating HTML page...")
    ...
    with open(
        os.path.join(DOWNLOAD_DIR, videoid, "index.html"), "w", encoding="utf-8"
    ) as f:
        f.write(
            template_content.format(
                videotitle=videotitle, filename=filename, icon=imagepath
            )
        )
    ...
    with open(VIDEO_LIST, "a", encoding="utf-8") as f:
        f.write(
            f'\t{videotitle} | <a href="videos/downloads/{videoid}/">videos/downloads/{videoid}/</a> | {timestamp}<br/>\n'
        )
