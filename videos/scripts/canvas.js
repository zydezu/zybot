class VideoWithBackground {
    ...
    initcheck = () => {
        ...
        console.log(
            `Initiated video canvas${isFirefox ? " (firefox version)" : ""}`,
        );
    ...
    checkThemeStatus = () => {
        ...
        if (
            currentTheme == "dark" &&
            localStorage.getItem("ambientMode") == "true"
        ) {
...
document
    .getElementById("ambientvideo")
    .addEventListener("loadeddata", checkResolution);

function checkResolution() {
    if (this.videoWidth / this.videoHeight < 1.6) {
        // 4/3 wont fit
